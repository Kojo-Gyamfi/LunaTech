...
function OrderConfirmationContent() {
  ...
  const orderDate = lastOrder?.createdAt ? new Date(lastOrder.createdAt) : fallbackOrderDate;
    ...
    <main className="min-h-screen bg-slate-950">
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-10 text-center">
          ...
          <p className="mb-2 text-base text-slate-400">Thank you for your purchase.</p>
          <p className="text-sm text-slate-500">
            Order ID:{' '}
            <span className="font-mono font-semibold">{orderId || lastOrder?.id || 'LUNA-DEMO'}</span>
          </p>
        </div>
        ...
        <div className="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="glass rounded-lg p-5 text-center">
            ...
            <h3 className="mb-1 text-sm font-semibold text-slate-100">Order Placed</h3>
          ...
          <div className="glass rounded-lg p-5 text-center opacity-70">
            ...
            <h3 className="mb-1 text-sm font-semibold text-slate-300">Processing</h3>
            <p className="text-xs text-slate-600">1-2 business days</p>
          </div>
          <div className="glass rounded-lg p-5 text-center opacity-70">
            <Truck size={30} className="mx-auto mb-3 text-slate-500" />
            <h3 className="mb-1 text-sm font-semibold text-slate-300">Delivery</h3>
            <p className="text-xs text-slate-600">{estimatedDelivery.toLocaleDateString()}</p>
          </div>
        </div>

        <div className="glass mb-8 rounded-lg p-5 sm:p-8">
          <h2 className="mb-6 text-xl font-bold text-slate-100 sm:text-2xl">Order Summary</h2>
          ...
          <div className="mb-6 space-y-4 border-b border-white/10 pb-6">
            {items.length > 0 ? (
              ...
              <p className="text-sm text-slate-400">
                Your order was placed successfully. Visit your account for order history.
              </p>
            )}
          </div>
          ...
          <div className="mb-6 space-y-2 text-sm">
            ...
            <div className="flex justify-between border-t border-white/10 pt-2 font-semibold text-slate-100">
              <span>Total</span>
              <span className="text-2xl text-amber-300">${total.toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3 rounded-lg border border-amber-500/20 bg-amber-500/10 p-4">
            ...
            <div>
              <p className="mb-1 font-semibold text-amber-300">Estimated Delivery</p>
              <p className="text-sm text-slate-400">
